...
public class FFT {
	...
	static {
		...
		_COS = new double[10000];
		_SIN = new double[10000];

		double theta = 0;
		dt = 2 * Math.PI / 10000;

		for (int i = 0; i < 10000; i++) {
			_COS[i] = Math.cos(theta);
			_SIN[i] = Math.sin(theta);
			theta += dt;
		}
	}

	public static double cos(double c) {
		return _COS[circleToIndex(c)];
	}

	public static double sin(double c) {
		return _SIN[circleToIndex(c)];
	}

	private static int circleToIndex(double c) {
		int count = (int) (c / (2 * Math.PI));
		if (c < 0) {
			c = c + (count + 1) * 2 * Math.PI;
		} else {
			c = c - count * 2 * Math.PI;
		}
		return (int) (c / dt);
